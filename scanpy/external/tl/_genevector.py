...
def genevector(
    ...
    **X_phate** : `np.ndarray`, (`adata.obs`, shape=[n_samples, n_components], dtype `float`)
    ...
    >>> sce.pl.phate(adata)
    """
    start = logg.info("running GeneVector")
    ...
    try:
        ...
        raise ImportError(
            "You need to install the package `genevector`: please run `pip install "
            "--user genevector` in a terminal."
        )
    ...
    logg.info(
        "    finished",
        time=start,
        deep=("added\n" "    'X_genevector', GeneVector coordinates (adata.obsm)"),
    )
